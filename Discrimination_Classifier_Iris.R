...
set.seed(10062017)
myTraining <- myData[sample(nrow(myData), 
                            replace = FALSE, size = ceiling(0.7*nrow(myData))),]

ID_Training <- row.names(myTraining)
ID_Data <- row.names(myData)
ID_Test <- which(!ID_Data%in%ID_Training)

myTest <- myData[ID_Test,]
# -------------------------------
# Linear Discriminant Analysis
# -------------------------------
myTrainLDA <- lda(Species ~., data = myTraining)
predictLDA <- predict(myTrainLDA, newdata = myTest)$class

LDA_error <- table(predictLDA==myTest[,5])/ nrow(myTest)

# -------------------------------
# QDA
# -------------------------------
myTrainQDA <- qda(Species ~., data = myTraining)
predictQDA <- predict(myTrainLDA, newdata = myTest)$class

QDA_error <- table(predictLDA==myTest[,5])/ nrow(myTest)

# -------------------------------
# Crossvalidation LDA QDA 
# -------------------------------
LDA.iris  <- lda(Species~., data=iris, CV=TRUE)
QDA.iris  <- qda(Species~., data=iris, CV=TRUE)


table(LDA.iris$class, iris$Species)
1 - sum(diag(table(LDA.iris$class, iris$Species)))/ sum(table(LDA.iris$class, iris$Species))

table(QDA.iris$class, iris$Species)
1 - sum(diag(table(QDA.iris$class, iris$Species)))/ sum(table(QDA.iris$class, iris$Species))
...
plot(logit2prob(logOdds))

myProb_IsVirginia <- logit2prob(logOdds)
